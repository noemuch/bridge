...
export async function extractHeadless(opts: ExtractOptions = {}) {
  ...
  await writeFile(path.join(outBase, "registries", "variables.json"), JSON.stringify(result.variables, null, 2) + "\n");
  await writeFile(path.join(outBase, "registries", "components.json"), JSON.stringify(result.components, null, 2) + "\n");
  await writeFile(path.join(outBase, "registries", "text-styles.json"), JSON.stringify(result.textStyles, null, 2) + "\n");
