...
test("planRegens orders: foundations → components → patterns → changelogs → migrations", () => {
  const writes = planRegens({
    ...
    migrations: [{ reason: "component-rename", from: "OldBtn", to: "Button", severity: "breaking" }],
  }, idx, "design-system");
...
test("planRegens slugifies migration filenames", () => {
  const writes = planRegens({
    componentsToRegen: [], foundationsToRegen: [], patternsToRegen: [], changelogsToAppend: [],
    migrations: [{ reason: "token-rename", from: "$color/bg/primary", to: "$color/brand/primary", severity: "breaking" }],
  }, idx, "ds");
