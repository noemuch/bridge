...
export function lintDoc(opts: {
  ...
  if (!fmMatch) {
    issues.push({ code: "frontmatter.missing", path: opts.path, message: "Missing YAML frontmatter" });
  ...
  for (const r of required) {
    if (!(r in (fm ?? {}))) {
      issues.push({ code: "frontmatter.required", path: opts.path, field: r, message: `Missing required field: ${r}` });
    }
  }
  const body = opts.content.slice(fmMatch[0].length);
  const tokenRefRe = /\$color\/[a-zA-Z0-9/_-]+|\$spacing\/[a-zA-Z0-9/_-]+|\$text\/[a-zA-Z0-9/_-]+|\$radius\/[a-zA-Z0-9/_-]+/g;
  const refs = body.match(tokenRefRe) ?? [];
  for (const ref of refs) {
    if (!(ref in opts.tokenIndex)) {
      issues.push({ code: "token.unresolved", path: opts.path, message: `Unresolved token reference: ${ref}` });
    }
  }
  const deeplinks = body.match(/https:\/\/[^\s)]*figma\.com\/[^\s)]+/g) ?? [];
  for (const link of deeplinks) {
    if (!link.includes("node-id")) {
      issues.push({ code: "figma.deeplink.shape", path: opts.path, message: `Figma link lacks node-id: ${link}` });
    }
  }
  return { path: opts.path, issues };
}
