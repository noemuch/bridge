import type { ComponentRegistry, VariableRegistry, TextStyleRegistry, Category } from "./registry-io.js";

export interface LearningsFile {
  learnings: Array<{ id: string; scope: string[]; frequency: number; agreedBy: number; promoted?: boolean; promotedAt?: string; }>;
  flags: unknown[];
}
...
export interface KbIndex {
  version: string;
  generatedAt: string;
  tokenIndex: Record<string, { category: "color" | "spacing" | "radius" | "text" | "other"; key: string; valuesByMode?: Record<string, unknown>; usedBy: string[]; }>;
  componentIndex: Record<string, { category: Category; status: string; key: string; uses: { tokens: string[]; components: string[] }; usedBy: string[]; alternatives: string[]; composesWith: string[]; supersedes: string[]; deprecatedBy: string | null; }>;
  learningIndex: Record<string, { scope: string[]; frequency: number; agreedBy: number; promoted: boolean; promotedAt?: string; }>;
...
export function buildFromScratch(kb: KbInput): KbIndex {
  ...
  for (const v of kb.variables.variables) {
    const ref = "$" + v.name;
    tokenIndex[ref] = { category: categorize(v.name), key: v.key, valuesByMode: v.valuesByMode, usedBy: [] };
  }
  ...
  for (const c of kb.components.components) {
    componentIndex[c.name] = {
      category: c.category, status: c.status, key: c.key,
      uses: { tokens: [], components: [] }, usedBy: [],
      alternatives: [], composesWith: [], supersedes: [], deprecatedBy: null,
    };
  }

  const learningIndex: KbIndex["learningIndex"] = {};
  for (const l of kb.learnings.learnings) {
    learningIndex[l.id] = { scope: l.scope, frequency: l.frequency, agreedBy: l.agreedBy, promoted: Boolean(l.promoted), promotedAt: l.promotedAt };
  }

  const patternIndex: KbIndex["patternIndex"] = {};
  for (const r of kb.recipes.recipes) {
    if (r.components && r.components.length > 0) patternIndex[r.name] = { components: r.components, recipes: [r.id] };
  }

  return { version: "3.2.0", generatedAt: new Date().toISOString(), tokenIndex, componentIndex, learningIndex, patternIndex };
}
